...
>
    <Select.Trigger
        ...
        aria-label="Chọn một hoặc nhiều"
    >
        {selectedLabel}
        <CaretDown
        ...
        >
            ...
            <Select.Viewport class="p-1">
                {#each items as item, i (i + item.value)}
                    <Select.Item
                        class="rounded-lg data-highlighted:bg-surface-tertiary outline-hidden data-disabled:text-content-muted flex h-fit w-full select-none items-center py-2 px-3 gap-3 text-base font-medium"
                        value={item.value}
                        label={item.label}
